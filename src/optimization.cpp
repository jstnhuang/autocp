...
void Optimization::ChooseViewpoint(const Viewpoint* nearby_point,
  ...
  for (auto& viewpoint : viewpoints) {
    Score score;
    ComputeViewpointScore(viewpoint, nearby_point, &score);
...
void Optimization::ComputeViewpointScore(const Viewpoint& viewpoint,
  ...
  // Travel score.
  if (ideal_viewpoint != NULL) {
    float travel_score = TravelingScore(viewpoint, *ideal_viewpoint);
...
float Optimization::TravelingScore(const Viewpoint& viewpoint,
                                  const Viewpoint& ideal_viewpoint) {
  auto distance = viewpoint.position().distance(ideal_viewpoint.position());
  return linearInterpolation(0, 1, max_travel_, 0, distance);
}
